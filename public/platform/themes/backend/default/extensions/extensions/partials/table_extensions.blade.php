			...
			@if ( ! array_get($extension, 'info.is_core'))
				...
				<div class="btn-group">
					@if (array_get($extension, 'info.has_update'))
						<a class="btn btn-info" href="{{ URL::to_secure(ADMIN.'/extensions/update/'.array_get($extension, 'info.slug')) }}" onclick="return confirm('Are you sure you want to update the \'{{ e(array_get($extension, 'info.name')) }}\' extension?');">{{ Lang::line('extensions::extensions.button.update') }}</a>
					@endif
