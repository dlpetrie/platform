...
<section id="extensions">
    <header class="row-fluid">
        <div class="span12">
            ...
            <p>{{ Lang::line('extensions::general.description.view', array('extension' => array_get($extension, 'info.name'))) }}</p>
        </div>
    ...
    {{ Form::open() }}
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ HTML::decode( HTML::link(ADMIN . '/extensions/view/' . array_get($extensions, $dependent . '.info.slug'), array_get($extensions, $dependent . '.info.name', $dependent) ) ) }}</td>
                    <td>{{ $dependent }}</td>
                    <td>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ HTML::decode( HTML::link(ADMIN . '/extensions/view/' . array_get($extensions, $dependent . '.info.slug'), array_get($extensions, $dependent . '.info.name') ) ) }}</td>
                    <td>{{ $dependent }}</td>
                    <td>
                        @if ( Platform::extensions_manager()->is_installed( $dependent ) )
                            <span class="label label-success">{{ Lang::line('extensions::table.installed')->get() }}</span>
        ...
        @if ( Platform::extensions_manager()->is_core( array_get($extension, 'info.slug') ) )
            ...
            @else
                <span class="label label-important">{{ Lang::line('extensions::messages.is_core')->get() }}</span>
            @endif
        @else
            @if ( Platform::extensions_manager()->is_installed( array_get($extension, 'info.slug') ) )
                @if ( Platform::extensions_manager()->can_uninstall( array_get($extension, 'info.slug') ) )
                    ...
                    @endif
                @else
                    <span class="label label-info">{{ Lang::line('extensions::messages.required')->get() }}</span>
                @endif
            @else
                @if ( ! Platform::extensions_manager()->can_install( array_get($extension, 'info.slug') ) )
                    <span class="label label-warning">{{ Lang::line('extensions::messages.requires')->get() }}</span>
                @else
                    <div class="btn-group">
