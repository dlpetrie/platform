@foreach ($rows as $row)
	<tr>
		...
		<td class="span2">
			<a class="btn" href="{{ URL::to_secure(ADMIN.'/users/edit/'.$row['id']) }}">{{ Lang::line('buttons.edit') }}</a>
			<a class="btn btn-danger" href="{{ URL::to_secure(ADMIN.'/users/delete/'.$row['id']) }}">{{ Lang::line('buttons.delete') }}</a>
		</td>
	</tr>
@endforeach
